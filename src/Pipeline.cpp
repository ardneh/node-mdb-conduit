...
Handle<Value> callWithError(HandleScope& scope, Local<Function>& callback, const char* const msg) {
...
Handle<Value> aggregate(const Arguments& args) {
	...
	struct Args { enum { PIPELINE, DOCUMENTS, CALLBACK, numArgs }; };
	...
	if (!args[Args::PIPELINE]->IsArray()) {
		...
		//Local<Value> error(Exception::TypeError(String::New("BAD PIPELINE, MUST BE AN ARRAY!")));
		//callback->Call(Context::GetCurrent()->Global(), 1, &error);
		//return scope.Close(Undefined());
	}

	if (!args[Args::DOCUMENTS]->IsArray()) {
		return callWithError(scope, callback, "BAD DOCUMENT STREAM, MUST BE AN ARRAY!");
	}
	...
	try {
		...
		//Update: recent tests are around ~60ms.  Not sure if it's improved that much
		//or if me test changed...
		Local<Value> results[2] = {
			Local<Value>::New(Undefined()),
			Local<Value>::New(String::New(result.obj().jsonString().c_str()))
		};
